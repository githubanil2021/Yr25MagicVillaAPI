    ...
    public class VillaNumberController : Controller
    {
        ...
        public async Task<IActionResult> CreateVillaNumber(VillaNumberCreateVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (response != null && response.IsSuccess && (response.ErrorMessages==null || response.ErrorMessages.Count==0))
                {
        ...
        public async Task<IActionResult> UpdateVillaNumber(VillaNumberUpdateVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (response != null && response.IsSuccess && (response.ErrorMessages == null || (response.ErrorMessages!=null  && response.ErrorMessages.Count == 0)))
                {
                    return RedirectToAction(nameof(IndexVillaNumber));
                }
                else
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteVillaNumber(VillaNumberUpdateVM model)
        {
            if (ModelState.IsValid)
            {
                var response = await _villaNumberService.DeleteAsync<APIResponse>(model.VillaNumber.VillaId, HttpContext.Session.GetString(SD.SessionToken));
                if (response != null && response.IsSuccess)
                {
                    return RedirectToAction(nameof(IndexVillaNumber));
                }
